...
def get_active_users() -> User:
    ...
    date_filter = datetime.today() - timedelta(days=10)
    return User.objects.filter(last_login__gt=date_filter)
...
def get_all_critical_events_by_user(agent) -> Event:
    ...
    return Event.objects.filter(level='critical', agent__id=agent.id)
